...
def bot_config():
    ...
    if "skill_id" in data:
        _bot.skill_id = int(data["skill_id"])
    return jsonify({"status": "ok", "config": {
        "delay_min": _bot.delay_min,
        "delay_max": _bot.delay_max,
        "harvest_duration": _bot.harvest_duration,
        "skill_id": _bot.skill_id,
    }})


@app.route("/api/debug")
def api_debug():
    """Route de diagnostic — accessible via /api/debug"""
    gs = _game_state.to_dict() if _game_state else {}
    bot = _bot.to_dict() if _bot else {}
    return jsonify({
        "bot_running":    bot.get("running"),
        "bot_state":      bot.get("state"),
        "player_cell":    gs.get("player_cell_id"),
        "player_id":      gs.get("player_id"),
        "resources_count": len(gs.get("resources", [])),
        "available_count": gs.get("available_count"),
        "logs_count":     len(_bot.logs) if _bot else 0,
    })
